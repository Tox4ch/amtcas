...
Обычный MTProxy поднимается на одном сервере и напрямую соединяется с Telegram. В условиях российских блокировок это работает ненадёжно: трафик от прокси до Telegram проходит через российских провайдеров и может быть заблокирован или замедлен.

**amtcas** строит двухзвенную цепочку:
...
Туннель между RU и мостом использует протокол **VLESS+XTLS-Reality** — для глубокой инспекции пакетов (DPI) он выглядит как обычный HTTPS к Google. Заблокировать его, не затронув легитимный трафик, практически невозможно.
...
| Протокол | Как видит DPI | Уязвимость |
|---|---|---|
| Открытый SOCKS5 | Нешифрованный прокси | Легко блокируется |
| WireGuard / OpenVPN | VPN-трафик | Блокируется по сигнатуре |
| TLS-туннель (самоподписанный) | Подозрительный TLS | Блокируется по отсутствию валидного сертификата |
| **VLESS+Reality** | **Легитимный HTTPS к google.com** | **Заблокировать = заблокировать Google** |

Reality не генерирует собственный сертификат. Вместо этого он «заимствует» реальное TLS-рукопожатие у `www.google.com`, включая валидную цепочку доверия. DPI видит полностью легитимный трафик.
...
- 🔍 **Проверка пути соединения** — после установки проверяет туннель и показывает схему с реальными IP
- 🔗 **Готовая ссылка** — `tg://proxy?...` выводится сразу после запуска
- 🔄 **Автообновление** — при каждом запуске проверяет GitHub на новую версию и предлагает обновиться
- 💾 **Самоустановка** — после первого запуска доступен глобально как команда `amtcas`
